...
useEffect(() => {
  ...
  const handleSuccess = (exIdx: number, setIdx: number) => {
    ...
    return;
        ...
        const seconds = typeof(restValue) === "string" ? parseInt(restValue) : restValue;
      ...
      {workout.exercises.map((exercise, exIdx) => (
        <div key={exIdx} className="bg-slate-100 rounded-lg p-3 space-y-2">
          ...
          {exercise.sets.map((set, setIdx) => {
                    ...
                    const doneStyles = "bg-gradient-to-r from-green-400 via-green-300 to-lime-300 shadow-lg animate-popbeat";
                    const skippedStyles = "bg-gray-300 text-gray-600 animate-pulse";
                    const pendingStyles = "bg-white";
                  
                    const containerClasses = `
                      flex items-center justify-between p-3 rounded-lg
                      ${status === "done" ? doneStyles : status === "skipped" ? skippedStyles : pendingStyles}
                    `;
